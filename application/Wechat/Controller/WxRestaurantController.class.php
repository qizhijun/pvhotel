...
class WxRestaurantController extends Controller {
	...
	public function updateOrder($openid,$orderid,$transactionid,$totalfee){
		...
		$orderOne = M("order_cy")->find($order_id);
	...
	private function sendPayMsg($payopenid,$cost,$shopname,$transactionid){
		...
		$mess="[支付凭证]您成功预订[电谷酒店]餐饮，消费人民币" . $cost . "元。[微信交易单号：" . $transactionid . "][支付时间：" . date("Y-m-d H:i:s", time()) . "]";
