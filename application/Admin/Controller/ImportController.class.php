...
class ImportController extends AdminbaseController {
	...
	public function detail(){
		...
		if($id){
			$data=M("salary_list");
			$count=$data->where(array("fileid"=>$id))->count();
	        $page = $this->page($count, 15);
	        $dataList=$data->where(array("fileid"=>$id))
	        	->limit($page->firstRow . ',' . $page->listRows)
	        	->order("is_status desc")
	        	->select();
	...
	public function sendNotice(){
		...
		->where("esl.fileid='".$fileid."' and em.status=1 and em.isdel=0 and esl.is_status = 0")
		->limit("5")
		->select();
	...
	public function sendtice(){
		...
		$salary_file = $this->data_modal->where(array("id"=>$fileid))->save(array(
			"status"=>2
		));
		if($salary_file){
			$resArr=array(
