...
//		$res即使菜单结果json
	...
	public function wx_message(){
		$wechat_message = M("wx_message")->join('ejd_fans on ejd_fans.id = ejd_wx_message.openid',"left")->where("ejd_wx_message.status = 0")->field("ejd_fans.nickname,ejd_wx_message.*")->select();
	...
	public function wx_delete(){
		$id = I("get.id");
		$status = I("get.status");
		if(M("wx_message")->where("id='".$id."'")->setField("status",$status) !== false){
			$this->success("删除成功！",U('wechat/wx_message'));
		}else{
			$this->error("删除失败！");
		}
	}
	...
	public function message_post(){
		if(IS_POST){
			...
			if($id){
				...
				$message = M("wx_message")->where("id=".$id)->save($data);
				if($message){
					$resArr = array(
						"success" => true,
						"msg" => "回复成功!"
