...
//		$roomNum = M("room")->where("id='".$id."'")->field("total_number")->find();
//		//从特殊表里查询特殊房量
//		$roomNumTe = M("room_price")->where("hotel_id = '".$id."' and type=2 and ctime = '".strtotime($CheckInDate)."'")->field("room_num")->find();
...
//		$this->assign("numTe1",$numTe1);
	...
	public function GetCanderData(){
		...
		if($xymcode){
			...
			$xymid = M("xym_code")->where(array('roomid'=>$id))->order("created desc")->find();
			$zhexym = M("xymcode")->where("id = '".$xymid['xymid']."' and endtime >= '".$CheckI."' and starttime <='".$CheckI."' and isdel=1")->order("id desc")->find();
		}
		//判断是否加套餐价格
		if($selRoom == "-1"){
			if($xymphone && $zhexym){
				...
				foreach($Check as $n){
					if($zhekou[$n]){
						if($zhexym['type'] == 1){
							$RoomD[$n] = $roomM['clevel_price0']*(ceil($zhekou[$n]))/10;
							if($pArr[$n]){
								$RoomD[$n] = $pArr[$n]*(ceil($zhekou[$n]))/10;
							}
						}else{
							$RoomD[$n] = $roomM['clevel_price0']-(ceil($zhekou[$n]));
							if($pArr[$n]){
								$RoomD[$n] = $pArr[$n]-(ceil($zhekou[$n]));
			...
			if($xymphone && $zhexym){
				...
				foreach($Check as $n){
					if($zhekou[$n]){
						if($zhexym['type'] == 1){
							$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
							$RoomD[$n] = ($this->price($price1,$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))*(ceil($zhekou[$n]))/10;
							if($pArr[$n]){
								$RoomD[$n] = ($this->price($pArr[$n],$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))*(ceil($zhekou[$n]))/10;
							}
							
						}else{
							$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
							$RoomD[$n] = ($this->price($price1,$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))-(ceil($zhekou[$n]));
							if($pArr[$n]){
								$RoomD[$n] = ($this->price($pArr[$n],$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))-(ceil($zhekou[$n]));
							}
						}
					}else{
						$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
				...
				$price3 = array_sum($RoomD);
				$price2 = $price3 * $roomnum;
			}else{
				foreach($price as $p){
					$pArr[$p['ctime']] = $p['price']+$RoomRp['bfservice'];
				}
		...
		$order_num = M("order")
		->where("roomid='".$roomtype."' and ((begtime <= '".$start."' and endtime >= '".$start."') or (begtime > '".$start."' and begtime <= '".strtotime($end)."')) and status > 0")
		->field("roomid,begtime,endtime,room_number")
		->select();
		$enddate = array();
	...
	function book_order(){
		if(IS_POST){
			...
			$xymcode = $_POST['xymcode'];
			if($xymcode){
				$xymphone = M("member")->where("phone=".$xymcode)->find();
				$xymid = M("xym_code")->where(array('roomid'=>$id))->order("created desc")->find();
				$zhexym = M("xymcode")->where("id = '".$xymid['xymid']."' and endtime >= '".strtotime($CheckInDate)."' and starttime <='".strtotime($CheckInDate)."' and isdel=1")->order("id desc")->find();
			}
			if(!$xymphone){
				$xymphone['phone'] = 1;
			}
			...
			$order_num = M("order")
			->where("roomid='".$id."' and ((begtime <= '".strtotime($CheckInDate)."' and endtime >= '".strtotime($CheckInDate)."') or (begtime > '".strtotime($CheckInDate)."' and begtime <= '".strtotime($CheckOutDate)."')) and status > 0")
			->field("roomid,begtime,endtime,room_number")
			->group("id")
			->select();
			$enddate = array();
	...
	public function bookajax(){
		...
		$xymphone = M("member")->where("phone=".$xymcode)->find();
		$xymid = M("xym_code")->where(array('roomid'=>$id))->order("created desc")->find();
		$zhexym = M("xymcode")->where("id = '".$xymid['xymid']."' and endtime >= '".strtotime($HDSDate)."' and starttime <='".strtotime($HDSDate)."' and isdel=1")->order("id desc")->find();
		$RoomRp = M("room_rp")->where(array("id" => $sel))->find();
		$roomM = M("room")->where("id='".$id."'")->find();
		$bfservice = json_decode($RoomRp['bfservice'],true);
		$pricerule = json_decode($RoomRp['pricerule'],true);
		$tmp = 0;
		foreach($bfservice as $key=>$vo){
		...
		if($sel=='-1'){
			if($xymphone && $zhexym){
				...
				foreach($newdate as $n){
					if($zhekou[$n]){
						if($zhexym['type'] == 1){
							$RoomP[$n] = $roomM['clevel_price0']*(ceil($zhekou[$n]))/10;
							if($pArr[$n]){
								$RoomP[$n] = $pArr[$n]*(ceil($zhekou[$n]))/10;
							}
						}else{
							$RoomP[$n] = $roomM['clevel_price0']-(ceil($zhekou[$n]));
							if($pArr[$n]){
								$RoomP[$n] = $pArr[$n]-(ceil($zhekou[$n]));
			...
			if($xymphone && $zhexym){
				...
				foreach($newdate as $n){
					if($zhekou[$n]){
						if($zhexym['type'] == 1){
							$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
							$RoomP[$n] = ($this->price($price1,$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))*(ceil($zhekou[$n]))/10;
							if($pArr[$n]){
								$RoomP[$n] = ($this->price($pArr[$n],$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))*(ceil($zhekou[$n]))/10;
							}
							
						}else{
							$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
							$RoomP[$n] = ($this->price($price1,$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))-(ceil($zhekou[$n]));
							if($pArr[$n]){
								$RoomP[$n] = ($this->price($pArr[$n],$pricerule,$RoomRp['pricetype'],$RoomRp['reserveitem'],date("Y-m-d",$CheckI),date("Y-m-d",$CheckO),$bfservice[5],$n,$time2))-(ceil($zhekou[$n]));
							}
						}
					}else{
						$price1 = $roomM['clevel_price0']+$RoomRp['bfservice'];
				...
				$price3 = array_sum($RoomP);
				$price2 = $price3 * $selRooms;
			}
