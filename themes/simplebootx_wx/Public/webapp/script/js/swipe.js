		...
		*/
		translate: function(dis, win, isRevert){
			...
			var isVertical = this.isVertical,
				...
				index = this.cur + dir,
				scale = isRevert ? 1 : (1 - Math.abs(0.2 * dis / win)).toFixed(6),
				scaleV = getValue((-dir * win * (1 - scale) / 5).toFixed(6));
		

			//if(index < 0) index += slideLength;
		    //if(index > slideLength - 1) index = 0;
			if (index < 0||index > slideLength - 1) { return true;}
			...
			$(this.slides[index]).find('.link_wrap .wz_le').addClass("removes");
